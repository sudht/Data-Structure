...
// 첫 번째 노드로 삽입하는 연산
void insertFirstNode(linkedList_h *L, int x) {
	listNode* newNode;
	newNode = (listNode*)malloc(sizeof(listNode));	// 삽입할 새 노드 할당
	newNode->data = x;								// 새 노드의 데이터 필드에 x 저장  
	newNode->link = L->head;
	L->head = newNode;
}
...
int main() {
	...
	printf("(1) 리스트에 [1],[3],[7] 노드 삽입하기! \n");
	insertLastNode(L, 1); insertLastNode(L, 3); insertLastNode(L, 7);
	printList(L); getchar();

	printf("(2) 리스트에서 [3] 노드 탐색하기! \n");
	p = searchNode(L, 3);
	if (p == NULL) printf("찾는 데이터가 없습니다.\n");
	else printf("[%d]를 찾았습니다.\n", p->data);
	getchar();

	printf("(3) 리스트의 [3] 뒤에 [5] 노드 삽입하기! \n");
	insertMiddleNode(L, p, 5);
	printList(L); getchar();

	printf("(4) 리스트에서 [7] 노드 삭제하기! \n");
	p = searchNode(L, 7);		// 삭제할 노드 위치 p를 찾음
